...
export const useUserStore = defineStore('user', () => {
  ...
  async function login(username, password) {
    try {
      ...
      const data = res.data || res;  // 兼容不同返回格式
      ...
      return { success: false, msg: e.response?.data?.msg || e.message || '登录失败' };
    }
  }
  ...
  async function register(username, password, phone) {
    try {
      ...
      return { success: false, msg: e.response?.data?.msg || e.message || '注册失败' };
    }
  }
